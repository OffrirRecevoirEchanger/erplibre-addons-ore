odoo.define('website.ore.participer.instance', function (require) {
    'use strict';

    require('web_editor.ready');
    let ParticiperForm = require('website.ore.participer');
...
odoo.define("website.ore.date_and_time", function (require) {
    "use strict";
...
odoo.define("website.ore.participer", function (require) {
    'use strict';
    ...
    app.controller('OffreDemandeService', ['$scope', function ($scope) {
        ...
        $scope.getDatabaseInfo = function () {
            ...
            } else if ($scope.model === "ore.offre.service") {
        ...
        $scope.submit_form = function () {
            ...
            ajax.jsonRpc(url, "call", copiedForm).then(function (data) {
                    ...
                    if (data.error) {
                        ...
                        $scope.error = "Empty data - " + "/ore/participer/form/submit";
                    } else {
                        ...
                        if ($scope.form_is_nouvelle_offre($scope.state)) {
                            $scope.submitted_url = `ore/ore_offre_service/${data.offre_service_id}`;
                        } else if ($scope.form_is_nouvelle_demande($scope.state)) {
                            $scope.submitted_url = `ore/ore_demande_service/${data.demande_service_id}`;
