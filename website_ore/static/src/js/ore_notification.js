odoo.define('website.ore_notification', function (require) {
    "use strict";
    ...
    let ORENotification = Widget.extend({
        // properties
        _audio: null,
        ...
        _onNotification: function (notifications) {
            ...
            for (let i = 0; i < notifications.length; i++) {
                ...
                if (channel === canal_membre_update) {
                ...
                } else if ([canal_notif_echange_new].includes(channel)) {
                    let data = message.data;
                    if (data.hasOwnProperty("type_notification")) {
                        ...
                        if (["Demande adhésion clan acceptée", "Demande adhésion clan refusée"].includes(data.type_notification)) {
                    ...
                    for (const [key, value] of Object.entries(data)) {
                        if (key === "membre_favoris_ids") {
                            ...
                            let offreService6 = $scope.getDatabaseInfo("ore.offre.service", field_id);
                            // console.debug("action to do " + String(value));
                            if (value[0][0] === 4) {
                                // let field_id = value[0][1];
                                $scope.$broadcast("notify_favorite", {
                                    "model": "ore.offre.service",
                                    "field_id": field_id,
                                    "status": true
                                });
                                ...
                                console.error("Not support value '" + value + "' from membre_favoris_ids channel '" + channel + "' model 'ore.offre.service'");
                            }
                        }
                    }
                    ...
                    for (const [key, value] of Object.entries(data)) {
                        if (key === "membre_favoris_ids") {
                            ...
                            let demandeService6 = $scope.getDatabaseInfo("ore.demande.service", field_id);
                            // console.debug("action to do " + String(value));
                            if (value[0][0] === 4) {
                                // let field_id = value[0][1];
                                $scope.$broadcast("notify_favorite", {
                                    "model": "ore.demande.service",
                                    "field_id": field_id,
                                    "status": true
                                });
                                ...
                                console.error("Not support value '" + value + "' from membre_favoris_ids channel '" + channel + "' model 'ore.demande.service'");
                            }
                        }
                    }
                }
                if ([canal_clan_creation_public].includes(channel)) {
                    // Search sections and add to last container
                    const sections = $('section.s_activites_recentes.o_ore_liste_clan');
                    if (sections.length > 0) {
                        // TODO this duplicate logic of html, generated by the server
                        let html = '<div class="container_explorer">\n' +
                            '                            <a class="activite btn" href="/ore/ore_clan/' + message.data.clan_new_id + '" >\n' +
                            '                                <img class="activite_img img-fluid" style="flex: 0 0 auto; margin-right: 1em; width: 100px;" src="' + message.data.membre_photo + '">\n' +
                            '                                <div class="infos">\n' +
                            '                                    <div class="activite_title">' + message.data.name +'</div>\n' +
                            '                                    \n' +
                            '                                    <div class="activite_auteur">' + message.data.membre_name +'</div>\n' +
                            '                                    \n' +
                            '                                    </div>\n' +
                        ...
                        sections.each(function() {
                            const container = $(this).find('div.container_explorer');
                            if (container.length > 0) {
                                lastContainer = container.last();
                            }
                        });

                        if (lastContainer) {
                            lastContainer.after(html);
                        }
                    } else {
                        console.debug("Ignore new clan notification, not in good page.")
                    }
                }
            }
            // });
    ...
    return {
        ORENotification: ORENotification,
    };
