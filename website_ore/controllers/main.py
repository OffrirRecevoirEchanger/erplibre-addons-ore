...
def find_distance_from_user(env, str_address):
    ...
    if not env.user or not env.user.active or not OSRM_URL or not str_address:
        ...
        return ""
    res_partner_cls = env["res.partner"]
    membre_id = res_partner_cls.search([("user_id", "=", env.user.id)])
    if not membre_id:
        return ""
...
class OREController(http.Controller):
    @http.route(
        ["/ore/ore_offre_service/<int:offre_service>"],
        type="http",
        auth="user",
        website=True,
    )
    def get_page_offre_service(self, offre_service=None, **kw):
        ...
        ore_offre_service_cls = env["ore.offre.service"]
        if offre_service:
            ore_offre_service_id = (
                ore_offre_service_cls.sudo().browse(offre_service).exists()
            )
            str_diff_time = self._transform_str_diff_time_creation(
                ore_offre_service_id.create_date
            )
        ...
        dct_value = {
            "ore_offre_service_id": ore_offre_service_id,
        ...
        return request.render(
            "website_ore.ore_offre_service_unit_liste_offre_et_demande_service",
            dct_value,
        )
    ...
    @http.route(
        [
            "/ore/get_info/all_offre_service",
        ],
        type="json",
        auth="user",
        website=True,
    ...
    @http.route(
        [
            "/ore/get_info/get_demande_service/<model('ore.demande.service'):demande_id>",
        ],
        type="json",
        auth="user",
        website=True,
    ...
    @http.route(
        [
            "/ore/get_info/get_echange_service/<model('ore.echange.service'):echange_id>",
        ],
        type="json",
        auth="user",
        website=True,
    ...
    @http.route(
        ["/ore/ore_demande_service/<int:demande_service>"],
        type="http",
        auth="user",
        website=True,
    )
    def get_page_demande_service(self, demande_service=None, **kw):
        ...
        dct_value = {}
        if demande_service:
            ore_demande_service_id = (
                ore_demande_service_cls.sudo().browse(demande_service).exists()
            )
            dct_value["ore_demande_service_id"] = ore_demande_service_id
            str_diff_time = self._transform_str_diff_time_creation(
                ore_demande_service_id.create_date
            )
        ...
        # Render page
        return request.render(
            "website_ore.ore_demande_service_unit_liste_offre_et_demande_service",
            dct_value,
        )

    @http.route(
        [
            "/ore/offre_service_ore_offre_service_and_demande_service_ore_demande_service_list"
        ],
        type="json",
        auth="user",
        website=True,
    )
    def get_offre_service_ore_offre_service_and_demande_service_ore_demande_service_list(
        self, **kw
    ):
        env = request.env(context=dict(request.env.context))
        ...
        ore_offre_service_ids = (
            ore_offre_service_cls.sudo()
            .search(
        ...
        offre_services = ore_offre_service_cls.sudo().browse(
            ore_offre_service_ids
        )
        offre_services_count = ore_offre_service_cls.sudo().search_count(
            [("website_published", "=", True)]
        )
        ...
        ore_demande_service_cls = env["ore.demande.service"]
        ore_demande_service_ids = (
            ore_demande_service_cls.sudo()
            .search(
                [("website_published", "=", True)],
                order="create_date desc",
                limit=3,
        ...
        demande_services = ore_demande_service_cls.sudo().browse(
            ore_demande_service_ids
        )
        demande_services_count = ore_demande_service_cls.sudo().search_count(
            [("website_published", "=", True)]
        )
        ...
        for ore_offre_service_id in offre_services:
            diff_time = timedate_now - ore_offre_service_id.create_date
            str_diff_time = humanize.naturaltime(diff_time).capitalize() + "."
            lst_time_diff_offre_service.append(str_diff_time)
        for ore_demande_service_id in demande_services:
            diff_time = timedate_now - ore_demande_service_id.create_date
            str_diff_time = humanize.naturaltime(diff_time).capitalize() + "."
        ...
        return request.env["ir.ui.view"].render_template(
            "website_ore.ore_offre_service_and_ore_demande_service_list_liste_offre_et_demande_service",
            dct_value,
        )

    @http.route(
        ["/ore/type_service_categorie/<int:type_service_categorie>"],
        type="http",
        auth="user",
        website=True,
    )
    def get_page_type_service_categorie(
        ...
        ore_type_service_categorie_cls = env["ore.type.service.categorie"]
        if type_service_categorie:
            ore_type_service_categorie_id = (
                ore_type_service_categorie_cls.sudo()
                .browse(type_service_categorie)
                .exists()
            )
        else:
            ore_type_service_categorie_id = None
        dct_value = {
            "ore_type_service_categorie_id": ore_type_service_categorie_id
        }

        # Render page
        return request.render(
            "website_ore.ore_type_service_categorie_unit_liste_type_service_categorie",
            dct_value,
        )

    @http.route(
        ["/ore/type_service_categorie_list"],
        type="json",
        auth="user",
        website=True,
    )
    def get_type_service_categorie_list(self, **kw):
        ...
        ore_type_service_categorie_ids = (
            ore_type_service_categorie_cls.sudo().search([]).ids
        )
        type_service_categories = ore_type_service_categorie_cls.sudo().browse(
            ore_type_service_categorie_ids
        )
        ...
        # Render page
        return request.env["ir.ui.view"].render_template(
            "website_ore.ore_type_service_categorie_list_liste_type_service_categorie",
            dct_value,
        )
    ...
    def get_page_participer(self, **kw):
        ...
        ore_type_service_categorie_cls = env["ore.type.service.categorie"]
        ore_type_service_categorie_ids = (
            ore_type_service_categorie_cls.sudo().search([])
        )

        dct_value = {"type_service_categories": ore_type_service_categorie_ids}

        # Render page
        return request.env["ir.ui.view"].render_template(
            "website_ore.ore_type_service_categorie_list_publication",
            dct_value,
        )
    ...
    def get_communaute_evenements(self, **kw):
        return request.env.ref("website_ore.ir_ui_view_communaute_1").render()
    ...
    def get_communaute_groupes(self, **kw):
        return request.env.ref("website_ore.ir_ui_view_communaute_2").render()
    ...
    def get_monactivite_echanges(self, **kw):
        return request.env.ref("website_ore.ir_ui_view_echanges").render()
    ...
    def get_monactivite_echange(self, **kw):
        return request.env.ref("website_ore.ir_ui_view_echange").render()
    ...
    def get_publier_offre(self, **kw):
        return request.env.ref("website_ore.ir_ui_view_publier_offre").render()
    ...
    def get_notification(self, **kw):
        return request.env.ref(
            "website_ore.ir_ui_view_messages_et_notifications"
        ).render()
    ...
    def get_monactivite(self, **kw):
        return request.env.ref("website_ore.ir_ui_view_mon_activite").render()
    ...
    def get_offresservice(self, **kw):
        return request.env.ref(
            "website_ore.ir_ui_view_offres_service"
        ).render()
    ...
    def get_demandesservice(self, **kw):
        return request.env.ref(
            "website_ore.ir_ui_view_demandes_service"
        ).render()
    ...
    def get_monactivite_monagenda(self, **kw):
        return request.env.ref("website_ore.ir_ui_view_mon_agenda").render()
    ...
    def get_personal_information(self, **kw):
        ...
        # Hack time for demo
        # month_bank_time = 0
        # v1 = http.request.env["ore.echange.service"].search(
        #     [
        ...
        # v2 = http.request.env["ore.echange.service"].search(
        #     [
    ...
    def ore_personal_information_form_submit(self, **kw):
        membre_id = self.get_membre_id()
        if type(membre_id) is dict:
            # This is an error
            return membre_id
    ...
    @http.route(
        [
            "/ore/get_membre_information/<model('res.partner'):membre_id>",
        ],
        type="json",
        auth="user",
        website=True,
    ...
    @http.route(
        [
            "/ore/get_info/offre_service/<model('res.partner'):membre_id>",
        ],
        type="json",
        auth="user",
        website=True,
    ...
    @http.route(
        [
            "/ore/get_info/demande_service/<model('res.partner'):membre_id>",
        ],
        type="json",
        auth="user",
        website=True,
    ...
    def get_info_list_membre(self, ore_id, **kw):
        my_favorite_membre_id = [
            a.membre_id.id
            for a in http.request.env.user.partner_id.res_partner_ids.membre_favoris_ids
        ]
    ...
    # @http.route(
    #     [
    #         "/ore/get_info/echange_service/<model('res.partner'):membre_id>",
    #     ],
    #     type="json",
    #     auth="user",
    #     website=True,
    ...
    @http.route(
        [
            "/ore/get_info/recevoir_service/<model('res.partner'):membre_id>",
        ],
        type="json",
        auth="user",
        website=True,
    ...
    def get_participer_workflow_data(self, **kw):
        ...
        res_partner_ids = (
            env["res.partner"].sudo().search([("id", "!=", membre_id.id)])
        )
        lst_membre = [
            {
                "title": a.nom,
        ...
        for a in ore_type_service_categorie_ids:
            sub_list = []
            obj_data = {
                "id": a.id,
                "tree_id": f"{a.id}",
    ...
    def get_help_data(self, **kw):
        ...
        dct_model_field_name = {
            ...
            for a in env["ore.workflow.state"]
            .sudo()
            .search(
                [
        ...
        if http.request.env.user.partner_id.id == 4:
            ...
            actual_membre_id = env["res.partner"].search(
                [("partner_id.user_ids", "=", env.user.id)]
            )
    ...
    def ore_participer_form_submit(self, **kw):
        ...
        membre_id = http.request.env.user.partner_id.res_partner_ids.id

        demande_service_id = None
        offre_service_id = None
        new_ore_echange_service = None
        ...
        if state_id.caract_offre_demande_nouveau_existante in (
            ...
            if kw.get("offre_service_id"):
                ...
                if (
                    ...
                    new_ore_service = request.env[
                        "ore.demande.service"
                    ].create(vals)
                    demande_service_id = new_ore_service.id
                    status["demande_service_id"] = demande_service_id
                elif (
                    state_id.caract_offre_demande_nouveau_existante
                    == "Nouvelle offre"
                ):
                    new_ore_service = request.env["ore.offre.service"].create(
                        vals
                    )
                    offre_service_id = new_ore_service.id
        ...
        if state_id.caract_echange_nouvel_existant == "Nouvel échange":
            ...
            new_ore_echange_service = request.env[
                "ore.echange.service"
            ].create(vals)
        ...
        if state_id.caract_valider_echange:
            ...
            if not new_ore_echange_service:
                if not kw.get("echange_service_id").get("id"):
                    ...
                    return status
                new_ore_echange_service = (
                    request.env["ore.echange.service"]
                    .sudo()
            ...
            if date_service or time_service:
                ...
                if new_ore_echange_service.date_echange != date_echange_float:
    ...
    @http.route(
        "/ore/submit/demande/publish/<model('ore.demande.service'):demande_id>",
        type="json",
        auth="user",
        website=True,
        csrf=True,
    ...
    @http.route(
        "/ore/submit/demande/supprimer/<model('ore.demande.service'):demande_id>",
        type="json",
        auth="user",
        website=True,
        csrf=True,
    ...
    @http.route(
        "/ore/submit/offre/publish/<model('ore.offre.service'):offre_id>",
        type="json",
        auth="user",
        website=True,
        csrf=True,
    ...
    @http.route(
        "/ore/submit/offre/supprimer/<model('ore.offre.service'):offre_id>",
        type="json",
        auth="user",
        website=True,
        csrf=True,
    ...
    @http.route(
        "/ore/submit/my_favorite",
        type="json",
        auth="user",
        website=True,
        csrf=True,
    )
    def ore_my_favorite_submit(self, **kw):
        # Send from participer website
        status = {}

        membre_id = http.request.env.user.partner_id.res_partner_ids
        ...
        if not id_record or not model_name:
            return {
                "error": (
                    ...
                    f" '/ore/submit/my_favorite'."
                )
            }

        if model_name == "res.partner":
            favoris_membre_id = http.request.env["res.partner"].browse(
                id_record
            )
            ...
            offre_service_id = http.request.env["ore.offre.service"].browse(
                id_record
            )
            if membre_id.id in offre_service_id.membre_favoris_ids.ids:
                ...
                status["id"] = offre_service_id.id
                status["is_favorite"] = True
        elif model_name == "ore.demande.service":
            ...
            msg_error = (
                f"/ore/submit/my_favorite model '{model_name}' not supported."
            )
            _logger.error(msg_error)
            status["error"] = msg_error
    ...
    def get_participer_member_from_ore(self, **kw):
        ...
        env = request.env(context=dict(request.env.context))
        res_partner_ids = env["res.partner"].sudo().search([])
        return {
            "list": [
                {
                    "text": a.nom,
    ...
    @http.route(
        [
            "/ore/type_service_sous_categorie_list",
            "/ore/type_service_sous_categorie_list/<int:categorie_id>",
        ],
        type="json",
        auth="user",
        website=True,
    ...
    def get_type_service_sous_categorie_list(self, categorie_id=None, **kw):
        ...
        ore_type_service_sous_categorie_cls = env[
            "ore.type.service.sous.categorie"
        ]
        if categorie_id:
            ore_type_service_sous_categorie = (
                ore_type_service_sous_categorie_cls.sudo().search(
        ...
        dct_value = {
            "type_service_sous_categories": ore_type_service_sous_categorie
        }

        # Render page
        return request.env["ir.ui.view"].render_template(
            "website_ore.ore_type_service_categorie_list_publication_sous_categorie",
            dct_value,
        )

    @http.route(
        [
            "/ore/template/votre_contact_full",
        ],
        type="http",
        auth="user",
        website=True,
    ...
    def get_template_votre_contact_full(self, **kw):
        # Render page
        return request.env["ir.ui.view"].render_template(
            "website_ore.template_votre_contact_full",
        )

    @http.route(
        [
            "/ore/template/offre_ou_demande_de_service_generic",
        ],
        type="http",
        auth="user",
        website=True,
