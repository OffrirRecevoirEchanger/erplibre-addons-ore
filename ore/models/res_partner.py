...
class ResPartner(models.Model):
    ...
    membre_favoris_ids = fields.Many2many(
        ...
        comodel_name="res.partner.favoris",
    ...
    def create(self, vals_list):
        status = super(ResPartner, self).create(vals_list)
        for stat in status:
            ir_attach_id = self.env["ir.attachment"].create(
                {
                    "name": f"logo_res_partner_{stat.id}",
                    "datas": stat.logo,
    ...
    def write(self, vals):
        ...
        for rec in self:
            if "logo" in vals.keys():
                ...
                logo = vals.get("logo")
                if rec.sudo().logo_attachment_id:
                    rec.sudo().logo_attachment_id.datas = logo
                else:
                    ir_attach_id = self.env["ir.attachment"].create(
                        {
                            "name": f"logo_res_partner_{rec.id}",
                            "datas": logo,
                            "res_model": "res.partner",
                            "res_id": rec.id,
                            "type": "url",
                    ...
                    rec.sudo().logo_attachment_id = ir_attach_id.id
