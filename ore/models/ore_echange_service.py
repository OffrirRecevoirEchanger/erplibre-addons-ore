    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if "offre_service" in vals.keys():
                offre_service_id = self.env["ore.offre.service"].browse(
            ...
            if "demande_service" in vals.keys():
                demande_service_id = self.env["ore.demande.service"].browse(
                    vals.get("demande_service")
                )
        ...
        if lst_notif_value:
            self.env["ore.echange.service.notification"].create(
                lst_notif_value
            )
        return res
